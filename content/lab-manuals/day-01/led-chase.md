...
> **Session:** Session 1C -- LED Chase Pattern: Loops That Do the Work
> **Audience:** All teams (student-facing)
> **Time Allotted:** 40 minutes
> **Team Size:** 3 students per station
> **Prerequisites:** Session 1B (Hello Matrix) completed -- LED on Pin 9 already wired
>
> Make three LEDs "chase" each other using a `for` loop -- and discover how one line of code can control multiple pins.
...
| Red LED (5mm) -- from Lab 1B | 1 | [ ] |
| **Yellow LED (5mm)** | **1** | **[ ] NEW** |
| **Green LED (5mm)** | **1** | **[ ] NEW** |
| 220 ohm resistors | 3 | [ ] (1 from Lab 1B, **2 NEW**) |
...
### Step 1 -- Wire Two More LEDs (15 min)
...
- **Green LED:** Arduino Pin 7 -> jumper -> 220 ohm resistor -> LED long leg (anode) -> LED short leg (cathode) -> GND
- **Yellow LED:** Arduino Pin 8 -> jumper -> 220 ohm resistor -> LED long leg (anode) -> LED short leg (cathode) -> GND
- **Red LED:** Arduino Pin 9 -> (already wired from Lab 1B)

#### Steps

1. **Place the yellow LED** on the breadboard. Long leg (anode) on one row, short leg (cathode) on a different row.
2. **Place a 220 ohm resistor** from the yellow LED's long leg row to a free breadboard row.
...
5. **Repeat steps 1-4** for the green LED, connecting to **Arduino Pin 7**.
6. **Verify:** All three LED short legs connect to GND. Each LED has its own 220 ohm resistor. Each resistor connects to a different pin (7, 8, 9).
...
- [ ] Green LED: Pin 7 -> resistor -> LED long leg -> LED short leg -> GND
- [ ] Yellow LED: Pin 8 -> resistor -> LED long leg -> LED short leg -> GND
- [ ] Red LED: Pin 9 -> resistor -> LED long leg -> LED short leg -> GND (from Lab 1B)
...
#### Code walkthrough -- the `for` loop

| Line | What it does | Why it matters |
| :--- | :--- | :--- |
| `for (int i = 7; i <= 9; i = i + 1)` | Start i at 7. Keep going while i <= 9. Add 1 each time. | The loop runs 3 times: i = 7, then 8, then 9 |
...
#### 2c -- Send to board and verify

> **EXPECTED RESULT:** LEDs light one at a time, chasing green -> yellow -> red -> yellow -> green, repeating smoothly.

| Test | Expected Result |
| :--- | :--- |
| Upload code | Compiles and uploads without errors |
| Watch LEDs | Green -> Yellow -> Red -> Yellow -> Green, repeating smoothly |
| Pattern is continuous | No long pauses between forward and backward |

---

### Step 3 -- Challenge Extensions (If you finish early)
...
| 1C: LED Chase Pattern | **TODAY** | For loops -- LED chase pattern |
| 1D: Button Input | COMING SOON | If/else -- the board makes decisions |
...
<- [Session 1B: Hello Matrix](/lab-manuals/day-01/hello-matrix/) | [Session 1D: Button Input](/lab-manuals/day-01/button-input/) ->