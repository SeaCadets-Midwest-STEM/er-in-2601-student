...
| :--- | :--- | :--- |
| 2004 I2C LCD display | 1 | Already connected at your station (20x4 character display) |
| LDR (light-dependent resistor) | 1 | Two-legged component with serpentine (wavy) pattern on top |
| 10k ohm resistor | 1 | Brown-black-orange-gold color bands |
| Red LED (5mm) | 1 | Long leg = anode (+), short leg = cathode (-) |
| 220 ohm resistor | 1 | Red-red-brown-gold color bands |
...
flowchart TD
    ...
    LDR --> MIDDLE["Junction -> Pin A0"]
    MIDDLE --> R10["10k ohm Resistor"]
    R10 --> GND1["GND"]
...
| 2 | Connect 10k ohm resistor from LDR leg 2 column to another column | Resistor bridges the two columns |
...
| 5 | Jumper wire: 10k ohm resistor column -> GND on Arduino | Bottom of divider grounded |
...
Now extend the circuit: connect an LED to Pin 9 through a 220 ohm resistor (same wiring as Day 1, Session 1B).
...
| 2 | Connect 220 ohm resistor from LED cathode column to another column |
...
- [ ] LDR voltage divider wired correctly (5V -> LDR -> A0 -> 10k ohm -> GND)
...
| LCD "Light:" line shows only 0 | LDR not connected to 5V, or 10k ohm resistor missing | Check wiring. Verify voltage divider: 5V -> LDR -> A0 -> 10k ohm -> GND |
| LCD "Light:" line shows only 1023 | LDR not connected to 10k ohm resistor, or resistor not grounded | Check that 10k ohm resistor connects from A0 junction to GND |
...
| LED very dim | 220 ohm resistor replaced with higher value, or LED reversed | Check resistor color bands. Verify LED polarity (long leg to Pin 9) |
