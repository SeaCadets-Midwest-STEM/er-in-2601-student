...
> **Session:** 3F -- Design Sprint Challenge
> **Duration:** 45 minutes
...
> **EXPECTED RESULT:** By the end of this lab, your team will have an STL file queued in OrcaSlicer for printing. The design must fit within the SOVOL SV06 ACE build volume (220 x 220 x 250 mm).
...
1. Open FreeCAD
2. File -> New Document
3. Save immediately: File -> Save As -> `Station__[TeamName]_Enclosure.FCStd`

#### Step 2: Create the base plate

1. Switch to **Part Design** workbench
2. In the tree view, right-click the body -> New Pad
3. Sketch on XY plane: draw a rectangle for the base plate
   - Suggested size: 200 x 90 mm (fits Arduino + breadboard side by side)
...
| :------ | :------- | :-------------- |
| USB port | Program and power | 12 x 8 mm at back edge |
| Serial port | Debugging | 6 x 4 mm near USB |
| Matrix display | Visible output | Match your matrix measurements + 2 mm |
| LDR sensor | Light exposure | Match LDR diameter + 4 mm |
| Power LED | Status indicator | 4 x 4 mm |
...
| Check | Status |
| :----- | :---- |
| Total width <= 220 mm (SOVOL SV06 ACE limit) | Verify |
| Total depth <= 220 mm | Verify |
| Total height <= 250 mm | Verify |
| Wall thickness >= 1.0 mm | Verify |
| Text (if any) depth >= 0.8 mm | Verify |
...
| Supports | As needed | Only where overhangs > 45 degrees |
...
- [ ] Design fits within SOVOL SV06 ACE build volume (220 x 220 x 250 mm)
- [ ] Print time <= 4 hours
...
1. In FreeCAD, use the **Draft** workbench -> Text tool
...
| Export to STL fails | Model has invalid geometry | Use Part -> Validate Shape in FreeCAD. Fix errors before exporting. |
...
| Model exceeds build volume | Design too large | Scale down. Check SOVOL SV06 ACE limits: 220 x 220 x 250 mm. |

---

## Key Concept Summary

**This is the DESIGN -> PROTOTYPE -> TEST -> ITERATE cycle that real engineers use.** Your first design will rarely be perfect. The goal is to learn from the physical prototype and improve.
...
Continue to [Day 4 Lab Manuals](/lab-manuals/day-04/)
